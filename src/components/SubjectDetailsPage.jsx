...
const SubjectDetailsPage = () => {
  ...
  const handleResourceAccess = async (type, unitNo = null, year = null) => {
    ...
    try {
      ...
      // Path: /maths/resources/{core_id}/{resource_type}
      ...
      if (response.data?.resource_url) {
        ...
        alert("Resource link not found in server response.");
      }
    } catch (err) {
      if (err.response && err.response.status === 404) {
        alert(`The ${type} for ${year || 'this unit'} hasn't been uploaded yet.`);
      } else {
  ...
  return (
    <div className="min-h-screen bg-[#fafaf9] font-sans pb-20 selection:bg-emerald-100">
      ...
      <nav className="bg-white/80 backdrop-blur-xl border-b border-stone-200/60 px-6 py-4 sticky top-0 z-[100] flex justify-between items-center shadow-sm">
        <button onClick={() => navigate(-1)} className="group p-2 bg-stone-100 rounded-xl hover:bg-emerald-50 transition-colors">
      ...
      {actionLoading && (
        <div className="fixed inset-0 z-[200] bg-stone-900/20 backdrop-blur-[2px] flex items-center justify-center">
          <div className="bg-white p-6 rounded-3xl shadow-2xl flex items-center gap-4">
            ...
            <span className="font-black text-xs uppercase tracking-widest">Fetching Link...</span>
          </div>
        </div>
      )}
      ...
      <main className="max-w-4xl mx-auto px-4 py-12">
        <div className="mb-12">
           <h3 className="text-[10px] font-black text-stone-400 uppercase tracking-[0.3em] mb-6 px-2">Global Archives</h3>
           <div className="grid grid-cols-2 gap-4">
              <button 
                onClick={() => setIsYearModalOpen(true)}
                className="flex items-center justify-between p-6 bg-white border border-stone-200 rounded-[2rem] hover:border-emerald-500 hover:shadow-xl transition-all group"
              >
              ...
              >
                <div className="flex items-center gap-4">
                  <div className="p-3 bg-stone-100 text-stone-600 rounded-xl group-hover:bg-stone-900 group-hover:text-white transition-all">
                    <BookOpen size={20}/>
                  </div>
        ...
        <div className="mt-16">
          <h3 className="text-2xl font-[1000] tracking-tighter text-stone-900 uppercase mb-8 px-2">Unit Vault</h3>
          {loadingUnits ? (
            <div className="flex justify-center py-24 opacity-30"><Loader2 className="animate-spin text-emerald-500" size={40}/></div>
          ) : (
                ...
                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} key={i} className="bg-white border border-stone-200 rounded-[2.5rem] p-6 md:p-8 hover:border-emerald-200 transition-all group shadow-sm">
                  <div className="flex flex-col md:flex-row md:items-center gap-8">
                    <div className="flex-1 flex gap-6 text-left">
                      <div className="w-14 h-14 shrink-0 rounded-2xl bg-stone-50 border border-stone-100 text-stone-950 flex items-center justify-center font-black text-lg group-hover:bg-emerald-600 group-hover:text-white transition-all">
                        {i + 1}
                      </div>
